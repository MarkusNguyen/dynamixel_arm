...
SYSTEM_PROMPT = (
    ...
    "Use move_arm_joints_deg to move all 5 arm joints at once when the user gives angles in degrees "
    "(preferred, since this app displays arm joints in degrees everywhere else), move_arm_joints if the "
    "user explicitly gives radians, set_single_joint_deg to move just ONE joint by its 1-based index "
    "(1=j1 ... 5=j5) while leaving the others unchanged, set_gripper_position to move ONLY the gripper "
    "(gripper values are always in radians, since it's a multi-turn actuator), move_to_cartesian_xyz to "
    "send end-effector coordinates, or retrieve_joint_states to read joint positions. "
...
def move_arm_joints_deg(joint_angles_deg: list[float], duration_sec: float = 2.0) -> str:
    """Move all 5 arm joints at once to target angles in DEGREES
    [j1, j2, j3, j4, j5]. Prefer this over move_arm_joints whenever the
    user gives angles in degrees (this app's scenes and sliders also use
    degrees for the arm joints)."""
    if len(joint_angles_deg) != 5:
        return f"Error: expected 5 joint values, got {len(joint_angles_deg)}."
    joint_positions_rad = [math.radians(a) for a in joint_angles_deg]
    ok = node.send_arm_only(joint_positions_rad, duration_sec)
    return f"Arm moved to {joint_angles_deg}° ({duration_sec}s)." if ok else "Error sending arm command."

def set_single_joint_deg(joint_index: int, angle_deg: float, duration_sec: float = 2.0) -> str:
    """Move a single arm joint to a target angle in DEGREES, leaving the
    other 4 joints at their current position. joint_index is 1-based
    (1=j1 ... 5=j5), matching how joints are usually referred to by the
    user (e.g. "joint 3" or "j3")."""
    if joint_index < 1 or joint_index > 5:
        return "Error: joint_index must be between 1 and 5."
    if not node.joint_states_received.wait(timeout=2.0):
        return "Error: timeout reading current joint states from /joint_states."
    pos = node.current_joint_positions
    if not all(j in pos for j in ARM_JOINTS):
        return "Error: not all arm joints are present in the latest /joint_states."
    current_rad = [pos[j] for j in ARM_JOINTS]
    current_rad[joint_index - 1] = math.radians(angle_deg)
    ok = node.send_arm_only(current_rad, duration_sec)
    if ok:
        return f"Joint {joint_index} moved to {angle_deg}°, other joints unchanged ({duration_sec}s)."
    return "Error sending arm command."
...
def send_gemini_message(prompt: str, history: list) -> tuple[str, list]:
    ...
    If the primary model returns 503 (overloaded), automatically retries
    with each model in GEMINI_FALLBACK_MODELS in order before giving up.
    Any other error is raised immediately without falling back, since a
    fallback model won't fix e.g. an auth or malformed-request error."""
    ...
    last_error = None
    for model_name in [GEMINI_MODEL_NAME] + GEMINI_FALLBACK_MODELS:
        try:
            chat = client.chats.create(
                model=model_name,
                config=types.GenerateContentConfig(
                    system_instruction=SYSTEM_PROMPT,
                    tools=[move_arm_joints, move_arm_joints_deg, set_single_joint_deg, set_gripper_position, move_to_cartesian_xyz, retrieve_joint_states],
                ),
                history=history,
            )
            ...
            raise  # not an overload error — no point trying other models
    raise last_error
...
def run_gemini_prompt(prompt: str) -> None:
    """Send `prompt` to Gemini exactly as if the user had typed it into
    the chat box, appending both the user turn and the reply to
    chat_history/gemini_history. Shared by the free-text chat input and
    the quick "5-joint" form below."""
...
with tab4:
    ...
    with st.expander("⚡ Chỉnh nhanh 5 khớp (gửi lệnh cho Gemini)", expanded=False):
        st.caption("Điền góc (độ) cho từng khớp rồi bấm gửi — sẽ tự tạo câu lệnh và gửi cho Gemini.")
        qcols = st.columns(5)
        quick_joints_deg = [
            qcols[k].number_input(f"j{k + 1} (°)", min_value=-180.0, max_value=180.0, value=0.0, step=1.0, key=f"quick_j{k}")
            for k in range(5)
        ]
        quick_duration = st.number_input("Thời gian (s)", min_value=0.2, max_value=30.0, value=2.0, step=0.5, key="quick_duration")
        if st.button("🚀 Gửi lệnh chỉnh 5 khớp", type="primary", use_container_width=True):
            joints_str = ", ".join(f"{v}" for v in quick_joints_deg)
            quick_prompt = (
                f"Move all 5 arm joints to [{joints_str}] degrees over {quick_duration} seconds."
            )
            run_gemini_prompt(quick_prompt)
